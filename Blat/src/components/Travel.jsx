...
const Travel = () => {
  ...
  const checkUserSubscription = async () => {
    const channelId = '170362'; // Assuming channelId is based on 17 March 1962 DOB of Kalpana Chawla
  ...
  const fetchMessages = async () => {
    try {
      const channelId = '170362'; // Assuming channelId is 'sports18'
  ...
  const handleJoinChannel = async () => {
    try {
      const response = await axios.post('http://localhost:5000/api/subscribeChannel', {
        channelId: '170362', // Replace with the channelId of the Travel channel
      ...
      if (response.status === 201) {
        ...
        console.log('User subscribed to the Travel channel');
      } else {
        console.error('Failed to subscribe to the Travel channel');
      }
    } catch (error) {
  ...
  const handleSendMessage = async () => {
    try {
      const channelId = '170362'; // Replace with actual channelId
